...
| Write-back NVMe→sharded HDD when idle | `kiroshi demote --from <nvme> --to '/mnt/user/Lubu*/Dataset' --coordinator <url> [--util 15 --sustain-min 30] [--now]` | Flushes an NVMe cache dir onto the HDD array in per-spindle shard layout, but leases are **withheld until the array is idle** (util ≤ `--util`% sustained `--sustain-min`). `*` = one shard per disk (LubuN↔diskN); placement is a stable hash (idempotent, spreads across all spindles). Writes direct `/mnt/diskN` paths by design (carries the `direct_disk_write` ack). Runner: `--task kiroshi.demote:run`. Gate needs HDD telemetry on the coordinator (Linux); without it it fails OPEN. See `docs/DEMOTE.md`. |
