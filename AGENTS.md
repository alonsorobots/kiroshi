...
| NAS layout | `kiroshi nas assess --root <dir>` / `benchmark` / `shard` | Assess reports shard balance; benchmark measures per-disk throughput; shard partitions a dataset across spindles. Run BEFORE seeding so the topology matches where data lives. |
