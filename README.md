...
## Quickstart (single box smoke test)
...
`KIROSHI_WRITE_ROOT`, `KIROSHI_CONFIG`.
...
| `kiroshi seed --demo N` / `--jobs file.jsonl` | Enqueue gigs. |
...
# Runner — see the NAS caveat below:
...
> **NAS caveat (important):** a Runner that reads/writes a **NAS over SMB cannot
> run as `LocalSystem`** — LocalSystem can't see the per-user credentials in
> Credential Manager, so authenticated UNC paths fail. Install such a Runner
> under a real user account that holds those credentials:
>
> ```powershell
> kiroshi service install --role runner --task mypkg:run --fixer auto `
>   --account '.\me' --password '<pw>' `
>   --read-root '\\nas\share_direct' --write-root '\\nas\share' --token <tok>
> ```
>
> Kiroshi **refuses** to install a NAS-bound Runner as LocalSystem unless you pass
> `--force`. Services don't inherit your shell's environment, so pass everything
> the task needs via `--token`, `--read-root/--write-root`, or `--env KEY=VALUE`.
