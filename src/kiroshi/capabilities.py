...
CAPABILITIES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "command": "kiroshi status --coordinator <url>",
        "when_to_use": "Quick health check. For per-sub-job detail use HTTP /jobs or /metrics/export.",
    ...
    },
    {
        ...
        "when_to_use": "Find specific sub-jobs on a large job without listing all of them — e.g. 'every failed sub-job whose error mentions PermissionError' or 'every subjob_id under shard_03/.*P08'.",
    ...
    {
        "name": "job.health",
        "purpose": "Paragraph-form diagnosis of ONE job: progress %, active advisories on its spindles, and recent errors — shaped to paste straight into an agent's context.",
        "command": "MCP tool: job_health(subjob_id)  (composes /groups + /advisories + /status)",
        ...
        "when_not": "For fleet-wide scheduling health use 'decisions.summary'; for a single sub-job's lifecycle use 'job.trace'.",
    },
    {
