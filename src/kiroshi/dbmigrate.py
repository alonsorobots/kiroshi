...
Migration is idempotent (no-op once ``subjobs`` exists) AND atomic: the whole
rename runs inside a single transaction, so a crash mid-migration rolls back
cleanly rather than leaving a half-migrated DB that ``needs_migration`` would
then refuse to repair.
"""
...
def migrate(conn: sqlite3.Connection) -> bool:
    ...
    The migration (all steps run in ONE transaction — all-or-nothing):
    ...
    disk_expr = "disk" if "disk" in old_cols else "NULL"
    grp_expr = "grp" if "grp" in old_cols else "NULL"

    # Run the entire rename atomically. sqlite3.executescript() implicitly
    # COMMITs any pending transaction, so we deliberately avoid it here and
    # drive every statement through execute() inside one explicit BEGIN. If
    # anything raises, we roll back — leaving the DB fully in the old schema
    # rather than a half-migrated state that needs_migration() can't repair.
    conn.execute("BEGIN")
    try:
        ...
        conn.execute(f"""
            INSERT OR IGNORE INTO subjobs
                (subjob_id, spec, job, disk, state, lease_id, runner_id, host,
                 ...
                 metrics, created_at)
            SELECT
                job_id, spec, {grp_expr}, {disk_expr}, state, lease_id,
                runner_id, host, attempts, leased_at, lease_deadline,
                completed_at, error, metrics, created_at
            FROM jobs
        """)

        # Step 2: drop old jobs table + its indexes (subjobs has the data now)
        for stmt in (
            "DROP TABLE IF EXISTS jobs",
            "DROP INDEX IF EXISTS idx_jobs_state",
            "DROP INDEX IF EXISTS idx_jobs_lease",
            "DROP INDEX IF EXISTS idx_jobs_completed",
            "DROP INDEX IF EXISTS idx_jobs_grp",
            "DROP INDEX IF EXISTS idx_jobs_disk",
        ):
            conn.execute(stmt)
