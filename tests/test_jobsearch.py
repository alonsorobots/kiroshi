...
def _make_store(tmpdir: str) -> JobStore:
    ...
    gigs = [
        {"subjob_id": "shard_01/a/1/CLIP_P089.npz", "spec": {}, "job": "camp-a"},
        {"subjob_id": "shard_01/a/2/CLIP_P08x.npz", "spec": {}, "job": "camp-a"},
        {"subjob_id": "shard_03/b/1/CLIP_P089.npz", "spec": {}, "job": "camp-b"},
        {"subjob_id": "shard_03/b/2/CLIP_P100.npz", "spec": {}, "job": "camp-b"},
        {"subjob_id": "shard_07/c/1/CLIP_P050.npz", "spec": {}, "job": "camp-b"},
    ]
    store.seed(gigs, job="mixed")
    # complete two of them so we can filter by state
    store.complete([
        {"subjob_id": "shard_01/a/1/CLIP_P089.npz", "status": "ok", "metrics": {}},
        {"subjob_id": "shard_03/b/1/CLIP_P089.npz", "status": "ok", "metrics": {}},
    ])
    # fail one with an error message
    store.complete([
        {"subjob_id": "shard_03/b/2/CLIP_P100.npz", "status": "error",
...
def test_regexp_matches_job_id_prefix():
    with _store_ctx() as store:
        rows = store.list_jobs(subjob_id_re="^shard_03/")
        ids = sorted(r["subjob_id"] for r in rows)
...
def test_regexp_matches_pattern_in_filename():
    with _store_ctx() as store:
        rows = store.list_jobs(subjob_id_re="P089")
        ids = sorted(r["subjob_id"] for r in rows)
...
def test_regexp_anchors_dont_match_too_broadly():
    with _store_ctx() as store:
        rows = store.list_jobs(subjob_id_re="P089\\.npz$")
...
def test_combined_with_state_filter():
    with _store_ctx() as store:
        ...
        rows = store.list_jobs(states=("done",), subjob_id_re="^shard_01/")
        ids = [r["subjob_id"] for r in rows]
...
def test_combined_with_group_filter():
    with _store_ctx() as store:
        rows = store.list_jobs(job="camp-b", subjob_id_re="^shard_03/")
        ids = sorted(r["subjob_id"] for r in rows)
...
def test_bad_regex_raises_re_error_not_sqlite_crash():
    with _store_ctx() as store:
        try:
            store.list_jobs(subjob_id_re="(")         # unbalanced paren
